...
#ifndef GRAPE_PARALLEL_LAYPH_SYNC_MESSAGE_MANAGER_H_
#define GRAPE_PARALLEL_LAYPH_SYNC_MESSAGE_MANAGER_H_
...
/**
 * @brief LayphAsyc message manager.
 *
 * The send and recv methods are not thread-safe.
 */
class LayphAsycMessageManager : public MessageManagerBase {
 public:
  LayphAsycMessageManager() : comm_(NULL_COMM) {}
  ~LayphAsycMessageManager() override {
    if (ValidComm(comm_)) {
      MPI_Comm_free(&comm_);
    }
  }
