...
Louvain::neigh_comm(int node) {
  ...
  for (int i=0 ; i<deg ; i++) {
    ...
    if (neigh!=node) {
      ...
      neigh_weight[neigh_comm] += neigh_w; // get K_i_in of node to neight_comm
    }
  }
}
...
Louvain::partition2graph_binary() {
  ...
  g2.node_size.resize(nbc);
  
  int max_com_size = 0;
  for (int node = 0 ; node < (qual->size) ; node++) {
    comm_nodes[renumber[qual->n2c[node]]].push_back(node);
    comm_weight[renumber[qual->n2c[node]]] += (qual->g).nodes_w[node];
    g2.node_size[renumber[qual->n2c[node]]] = qual->com_size[qual->n2c[node]];
    if(max_com_size < qual->com_size[qual->n2c[node]]){
      max_com_size = qual->com_size[qual->n2c[node]];
    }
  }
  cerr << "  max_com_size=" << (max_com_size) << std::endl;
  
  for (int comm=0 ; comm<nbc ; comm++) {
    map<int,long double> m; // Count the neighborhood list of the community
...
bool
Louvain::one_level(const int max_comm_size) {
  ...
  do {
    ...
    for (int node_tmp = 0 ; node_tmp < qual->size ; node_tmp++) {
      ...
      long double w_degree = (qual->g).weighted_degree(node); // K_i
      ...
      for (int i=0 ; i<neigh_last ; i++) {
        if(qual->com_size[neigh_pos[i]] + (qual->g).node_size[node] > max_comm_size){ // method1: set max community number
          continue;
        }
      ...
      // cerr << "--debug..node_com=" << node_comm << " best_com=" << best_comm << endl;
      qual->insert(node, best_comm, best_nblinks);
      // cerr << "finish insert..." << endl;
  ...
  cerr << "  nb_pass_done=" << nb_pass_done << endl;

  // Statistics superpoint size distribution data
  cerr << "  start count:" << endl;
  int step = 500;
  int max_num = 1e6;
  vector<int> s(max_num/step+1, 0);
  for (int i=0 ; i < qual->com_size.size(); i++) {
    if(qual->com_size[i]/step < max_num/step){
      s[qual->com_size[i]/step]++;
    }
    else{
      s[max_num/step]++;
    }
  }
  for(int i = 0; i < max_num/step+1; i++){
    if(s[i] > 0){
      cerr << "   statistic: " << (i*step) << "-" << (i*step+step) << ":" << s[i] << endl;
    }
  }
  return improvement;
}
